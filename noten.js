...
function fillPupilsList(pupilsTable, pupilsCount, firstCell){
	...
	for(let i = 1; i <= pupilsCount; ++i){
		if(pupilsTable.querySelector(`tr:nth-child(${i})`).textContent.length > 0){
		pupilsList.innerHTML += `<option>${pupilsTable.querySelector(`tr:nth-child(${i})`).textContent}</option>`;
		}
...
function addEventListenerToMarkCell(){
	let mc = document.querySelectorAll('#xgrid .xcell.c-1-1 td[class*="cell--2-"]');
	console.log(mc);
	mc.forEach( element => {
...
let observerMarks = new MutationObserver( () => {
    ...
    observerMarks = new MutationObserver( () =>{
    	let pupilsCount = pupilsTable.childElementCount - 1;
...
observerMarks.observe(document.querySelector('body'), {childList: true, subtree: true});

document.querySelector('#xgrid .xcell.c-0-1').addEventListener('click', event => {
